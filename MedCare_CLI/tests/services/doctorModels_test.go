...
func TestDoctorFunctions(t *testing.T) {
	...
	t.Run("GetDoctorByID Success", func(t *testing.T) {
		...
		mockDB.Mock.ExpectQuery("SELECT user_id, specialization, experience, rating FROM doctors WHERE user_id = ?").
			WithArgs(userID).
			WillReturnRows(rows)

		doctor, err := services.GetDoctorByID(userID)
	...
	t.Run("GetAllDoctors Success", func(t *testing.T) {
		...
		mockDB.Mock.ExpectQuery("SELECT user_id, specialization, experience, rating FROM doctors").
			WillReturnRows(rows)
	...
	t.Run("UpdateDoctorExperience Success", func(t *testing.T) {
		...
		mockDB.Mock.ExpectExec(regexp.QuoteMeta("UPDATE doctors SET experience = ? WHERE user_id = ?")).
			WithArgs(experience, userID).
			WillReturnResult(sqlmock.NewResult(1, 1))

		// Call the UpdateDoctorExperience function
		err := services.UpdateDoctorExperience(userID, experience)
		assert.NoError(t, err)
	...
	t.Run("UpdateDoctorSpecialization Success", func(t *testing.T) {
		...
		// Expect the exact SQL query with regexp.QuoteMeta
		mockDB.Mock.ExpectExec(regexp.QuoteMeta("UPDATE doctors SET specialization = ? WHERE user_id = ?")).
			WithArgs(specialization, userID).
			WillReturnResult(sqlmock.NewResult(1, 1))

		// Call the UpdateDoctorSpecialization function
		err := services.UpdateDoctorSpecialization(userID, specialization)
		assert.NoError(t, err)
	...
	t.Run("ViewDoctorSpecificProfile Success", func(t *testing.T) {
		...
		rows := mockDB.Mock.NewRows([]string{"specialization", "experience", "rating"}).
			AddRow(expectedSpecialization, expectedExperience, expectedRating)

		// Expect the exact SQL query
		mockDB.Mock.ExpectQuery(regexp.QuoteMeta("SELECT specialization, experience, rating FROM doctors WHERE user_id = ?")).
			WithArgs(userID).
			WillReturnRows(rows)
		...
		// Call the function
		services.ViewDoctorSpecificProfile(userID)
